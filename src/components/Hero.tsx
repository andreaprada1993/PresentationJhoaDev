        ...
        <section className="relative min-h-screen flex items-center pt-[80px] overflow-hidden" id="home">
            ...
            <div className="container flex flex-col justify-center items-center text-center relative z-10 w-full">
                <div className="max-w-[800px] mx-auto animate-fade-in">
                    ...
                    <p className="text-[clamp(1rem,2vw,1.25rem)] text-text-secondary mb-10 max-w-[700px] mx-auto" dangerouslySetInnerHTML={{ __html: heroContent.description.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }}>
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-6 w-full">
                        {heroStats.map((stat, idx) => (
                            <div key={idx} className="p-6 text-left transition-transform duration-300 ease-in-out hover:-translate-y-1.5 glass-panel">
                                <h3 className="text-2xl mb-2 gradient-text">{stat.title}</h3>
                                <p className="text-text-secondary text-[0.95rem]">{stat.description}</p>
                            </div>
                        ))}
                    </div>
                </div>
